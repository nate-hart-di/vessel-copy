...
add_filter('vehicle_classes_attribute', function($classes, $view, $vehicle) {
        ...
        <ul class="<?= $post_type; ?> posts list-unstyled clearfix <?= ($thumbnail =="true" ? 'withThumbnail ' : '') . ($text != "light" ? $text : "") ?>">
            <?php while ($query->have_posts()) : $query->the_post();
                ...
                <li class="post">
                    ...
                    <?php }
                    if ($thumbnail == 'true') { ?>
                        <span class="post-image"><a trid="c37c7c29ae674a37aa074c" trc href="<?= get_permalink() ?>"><?= get_the_post_thumbnail( $id, array('auto',50)) ?></a></span>
                    <?php } ?>
