...
add_action('wp_enqueue_scripts', function () {
  if (function_exists('load_vehicle_sprites')) {
    load_vehicle_sprites(['mercedes-menu', 'sprinter-menu']);
  }
});
...
{
  return preg_match(
    '/stock|media.carbook|carshot|d31ufzz2yw5eti\.cloudfront\.net|stock-images|chrome|media.chromedata.com/i',
    $image_url,
  ) > 0;
}
...
//disabled due to client not using mb trim/models
...
add_filter(
  'vehicle_classes_attribute',
  function ($classes, $view, $vehicle) {
            ...
            <?php while ($query->have_posts()):
              ...
              ?>
                ...
                <li class="post">
                    ...
                    <?php }
                    if ($thumbnail == 'true') { ?>
                        <span class="post-image"><a trid="c37c7c29ae674a37aa074c" trc href="<?= get_permalink() ?>"><?= get_the_post_thumbnail(
  $id,
  ['auto', 50],
) ?></a></span>
                    <?php }
                    ?>
                    <h3><a trid="911298f8518644e1b5a670" trc href="<?= get_permalink() ?>"><?= get_the_title() ?></a></h3>
                    <?php
...
add_action('mb_additional_phone_number', function () {
  ?>
        |<span class="phone"> Parts: <?php echo do_shortcode(
          '[di_phone option_key="parts" format="withparens" clickable="true"]',
        ); ?> </span>
    <?php
});
