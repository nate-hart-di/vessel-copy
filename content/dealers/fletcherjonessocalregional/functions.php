...
function add_horsepower_to_inventory($vehicle)
{
  ...
  if ($apiID == 'FJAudiBeverlyHills') {
    $vehicle->VRP_link =
      '<a trid="d15f5067030d47a58a6993" trc target="_new" class="vehicle-location-link" href="' .
      rtrim($location['url'], '/') .
    ...
    $vehicle->VRP_link =
      '<a trid="440c3723ca624d66bb98fe" trc target="_new" class="vehicle-location-link" href="' .
      rtrim($location['url'], '/') .
	...
	<div id="dealer-location-popover" class="hidden">
		<div class="row location-popover-wrap">
			...
			<?php if ($location['url'] !== false && !empty($location['url'])): ?>
				<a trid="3ee4a3d6b9864930ad8411" trc class="btn btn-sm btn-block btn-primary" target="_blank" href="<?= $location[
      'url'
  ...
  if (isset($years)) {
    ...
    foreach ($years as $year):
      ...
      foreach ($months as $month):
        ...
        $archive_results_html .=
          '<li><a trid="b8cc5b8bb81d48979141ea" trc href="' .
          $month_link .
          '"><span>' .
          $link_date .
          '</span></a></li>';
      ...
      ($text != 'light' ? $text : '') ?>">
        <?php while ($query->have_posts()):
          ...
          ?>
            ...
            <?php }
            if ($thumbnail == 'true') { ?>
                <span class="post-image"><a trid="9fc33e24500c4bcf8242e2" trc href="<?= get_permalink() ?>"><?= get_the_post_thumbnail(
  $id,
  ['auto', 50],
) ?></a></span>
            <?php }
...
add_filter('inventory-display-vrp-sorter', function ($sorts) {
  $mapping1 = new SortBuilder_Mapping(1, ['key' => 'make', 'value' => 'Mercedes-Benz']);
  $mapping2 = new SortBuilder_Mapping(1, ['key' => 'make', 'value' => 'Audi']);
  $mapping3 = new SortBuilder_Mapping(1, ['key' => 'make', 'value' => 'Porsche']);
  $group = new SortBuilder_GroupBy('sort_make', 'asc', 2, $mapping1, $mapping2, $mapping3);
  $order = new SortBuilder_OrderBy('our_price', 'asc'); // Builds a sort, All Buick Enclaves first, then any other Buicks, then all other vehicles, sorted by price from low to high
...
add_action('wp_footer', function () {
  if (method_exists('\DIFunctions', 'is_vehicle_page') && \DIFunctions::is_vehicle_page()): ?>
    <div id="schedule-test-drive" class="hidden-all">
        <?= do_shortcode(
          '[gravityform id="7" name="Schedule Test Drive" title="false" description="true" field_values="model_year={year}&make={make}&model={model}&trim={trim}&vin={vin}&link={link}&stock={stock}&type={type}&our_price={our_price}&original_price={original_price}&dealer_id={dealer_id}" ]',
        ) ?>
    </div>
