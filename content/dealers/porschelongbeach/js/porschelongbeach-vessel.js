  ...
  if(isVRP.any()) {
      $('body').on('vrp-ready vrp-ajax-complete', function() {
        $('.vehicle').each(function(i, obj) {
         ...
         $(this).find($("a.cta-button.button.roadster-btn.icon.get-eprice.block.green.button-primary")).appendTo($(this).find(".advanced-pricing-stack"));
         $(this).find(".button.primary-button.block.options-button").appendTo($(this).find(".vehicle-leftcol .vehicle-badges"));
        });
        if($(document).width() < 768 && $('.show-filters-button').length === 0) {
          var sort = $('.sorting-options');
          $(sort).removeClass("hidden-xs hidden-phone").append("<a trid='6faa7c0b092d4d35804b95' trc class='button primary-button show-filters-button align-left' style='margin:0'>Filter Results</a>").find('select').css({width:"40%"});
