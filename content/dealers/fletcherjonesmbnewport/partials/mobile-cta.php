...
<div class="mobile-cta mobile-cta-inside-page visible-xs">
	<?php if (have_rows('mobile_single_cta')):
   while (have_rows('mobile_single_cta')):
     ...
     while (have_rows('ctas')):
       ...
       ?>
				<div class="ctaItem">
					...
					<h2 class="cta-title <?php if ($ctaTitleColor != '#ffffff'):
       echo 'dark';
     endif; ?>" style="color:<?= $ctaTitleColor ?>"><?= $title ?></h2>
					<a trid="dfce5c04284b49ffa1863c" trc <?php if (
       $ctaClass
     ) { ?> href="#" <?php } else { ?> href="<?php echo $ctaLink; ?>" <?php } ?> <?php echo $new_window; ?> class="button dark-button <?php if (
   $ctaClass
 ):
   echo $ctaClass;
 endif; ?>" style="background-color:<?= $ctaLinkBG ?>;color:<?= $ctaLinkColor ?>"><?= $ctaLinkTitle ?></a>
				</div>
			<?php
     endwhile;
   endif; ?>
