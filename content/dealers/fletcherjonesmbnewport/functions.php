...
function dealer_theme_inventory_cleanup($vehicle)
{
  ...
  if (!empty($vehicle->premium_options) && !empty($vehicle->packages)) {
    ...
    $packages = [
      'PREMIUM 1 PACKAGE' => ['PREMIUM 1 PACKAGE', 'PREMIUM PACKAGE', 'PREMIUM I PACKAGE'],
      'PREMIUM 2 PACKAGE' => ['PREMIUM 2 PACKAGE', 'PREMIUM PACKAGE II'],
    ];
...
function search_terms_alias($s)
{
  ...
  if (method_exists($s, 'where') && !empty($s->search_term)) {
    ...
    if (
      !is_stock_prefix($search_term) &&
      preg_match('/([a-zA-Z]{1,3})(?:\s|-)?(\d{2,3})/', $s->search_term, $matches)
    ) {
      ...
      $model = null;
      $modified_search_term = preg_replace_callback(
        '/(?<=\s|^)(?<model>[a-zA-Z]{1,3})(?:\s{1,}|-)class(?=\s|$)/i',
        function ($matches) use (&$model) {
          $model = sprintf('%s%s', strtoupper($matches['model']), strlen($matches['model']) == 1 ? '-Class' : '');
    ...
    if (!empty($model)) {
      $s->where('model', is_string($model) ? '=' : 'IN', $model);
    }

    if (!empty($trim)) {
      $s->where('trim', is_string($trim) ? 'LIKE' : 'IN', $trim);
    }
...
function locked_pricing_vehicle_text()
{
  global $inventoryFrontend;
  if (
    $inventoryFrontend->vehicle['type'] == 'New' &&
...
add_filter(
  'di_locked_pricing_price_selector',
  function ($price_selectors, $obj) {
...
function pricing_bottom_text_customization()
{
  ...
  if ($inventoryFrontend->vehicle->type == 'New') {
    ...
    if (
      ...
      $inventoryFrontend->vehicle->year == 2018
            ...
            <div id="eoy-disclaimer">2 Month Payment Waiver!*</div><?php endif;
    //EoF End of year disclaimer

    // Lease offer CTA
    ?><a trid="734c207449db4a73a9680d" trc href="#gravity-form-101" id="lease-for-less-cta" class="button-form block button primary-button fancy" data-form-id="101" data-vehicle='<?php echo json_encode(
  $inventoryFrontend->data['vehicle']['json'],
  JSON_FORCE_OBJECT,
); ?>'>Get Your Lease Offer Now</a>
        <?php add_form_vrp(get_form_id('Get Your Lease Offer Now'));
        //EoF Lease Offer CTA
  ...
  if (shouldShowEPrice($inventoryFrontend->vehicle->model, $inventoryFrontend->vehicle->type)): ?>
        <a trid="7177584b2f3e41fdb1ddce" trc href="#gravity-form-1" id="get-eprice-cta" class="true button-form button cta-button block fancy button-form" data-form-id="1" data-vehicle='<?php echo json_encode(
          $inventoryFrontend->data['vehicle']['json'],
          JSON_FORCE_OBJECT,
        ); ?>'>Get E-Price</a>
    <?php endif;?>
...
{
  add_filter(
    'im_vrp_gravity_forms',
    function ($vrp_forms) use ($id) {
...
function vrp_inquire_cta()
{
  ...
  if (
    ...
    ($veh_model == 'g-class' || $veh_model == 's-class' || $veh_model == 'amg' || $veh_model == 'sl-class')
  ): ?>
...
function vdp_inquire_cta()
{
  ...
  if (
    ($frontend->vehicle->type == 'new' && $veh_model == 'g-class') ||
    $veh_model == 's-class' ||
    $veh_model == 'amg' ||
    $veh_model == 'sl-class'
  ): ?>
...
function add_class_to_specific_models($classes, $view, $vehicle)
{
  if ($vehicle['type'] == 'New') {
    if (
      ...
      strtolower($vehicle['model']) == 'g-class' ||
...
function c_class_lock_in_price_cta($cta_buttons)
{
  ...
  if (
    $inventoryFrontend->data['vehicle']['type'] !== 'New' &&
    $inventoryFrontend->data['vehicle']['model'] == 'C-Class'
  ) {
...
function lightning_vehicle_finder()
{
  ?><script>
    ...
    global $post;
    if (
      get_post_meta($post->ID, '_wp_page_template', true) == 'templates/page-lightning.php'
            ...
            <?php while ($query->have_posts()):
              ...
              ?>
                ...
                <li class="post">
                    ...
                    <?php }
                    if ($thumbnail == 'true') { ?>
                        <span class="post-image"><a trid="eaf44afc5e6b4f818bc17f" trc href="<?= get_permalink() ?>"><?= get_the_post_thumbnail(
  $id,
  ['auto', 50],
) ?></a></span>
                    <?php }
...
function disable_conversations_globally($bool)
{
  ...
  foreach ($pages as $page) {
    ...
    $page_id_array = [$service_page_id];
  }

  // check if current page is in list of pages, as well as any child pages
  if (
    isset($post->ID) &&
    (in_array($post->ID, $page_id_array) || ($post->post_parent && in_array($post->post_parent, $page_id_array)))
  ) {
    return false;
  }
  return $bool;
...
add_filter('body_class', function ($classes) {
  ...
  if (
    DIFunctions::is_vehicle_page() === true &&
    $inventoryFrontend->vehicle['next_to_new'] &&
    $inventoryFrontend->vehicle['api_id'] == 'FJMotorcarscode9'
  ) {
