...
function add_horsepower_to_inventory($vehicle)
{
  ...
  if ($apiID == 'FJAudiBeverlyHills') {
    $vehicle->VRP_link =
      '<a trid="a02129adce77488c8da172" trc target="_new" class="vehicle-location-link" href="' .
      rtrim($location['url'], '/') .
    ...
    $vehicle->VRP_link =
      '<a trid="fe57ee4876e44c0c8520d5" trc target="_new" class="vehicle-location-link" href="' .
      rtrim($location['url'], '/') .
	...
	<div id="dealer-location-popover" class="hidden">
		<div class="row location-popover-wrap">
			...
			<?php if ($location['url'] !== false && !empty($location['url'])): ?>
				<a trid="411d78d07cbe44c3ac83eb" trc class="btn btn-sm btn-block btn-primary" target="_blank" href="<?= $location[
      'url'
  ...
  if (isset($years)) {
    ...
    foreach ($years as $year):
      ...
      foreach ($months as $month):
        ...
        $archive_results_html .=
          '<li><a trid="6210bc7063c84e39aa9b65" trc href="' .
          $month_link .
          '"><span>' .
          $link_date .
          '</span></a></li>';
      ...
      ($text != 'light' ? $text : '') ?>">
        <?php while ($query->have_posts()):
          ...
          ?>
            ...
            <?php }
            if ($thumbnail == 'true') { ?>
                <span class="post-image"><a trid="5ec723e39c86424eaf4512" trc href="<?= get_permalink() ?>"><?= get_the_post_thumbnail(
  $id,
  ['auto', 50],
) ?></a></span>
            <?php }
...
register_nav_menus(['mobile-footer-menu' => 'Mobile Footer Menu', 'mobile-header-menu' => 'Mobile Header Menu']);
