...
function add_horsepower_to_inventory($vehicle)
{
  ...
  if(isset($vehicle->tech_options) && !empty($vehicle->tech_options)) {
	...
	if($apiID == 'FJAudiBeverlyHills') {
		$vehicle->VRP_link = '<a trid="a02129adce77488c8da172" trc target="_new" class="vehicle-location-link" href="'. rtrim($location['url'], "/").'/all-inventory/index.htm?search='.$vehicle->vin.'">'.$location['name'].'<br/>'.$location['phone'].'</a>';
	}
	else {
		$vehicle->VRP_link = '<a trid="fe57ee4876e44c0c8520d5" trc target="_new" class="vehicle-location-link" href="'.rtrim($location['url'], '/').'/new-vehicles/#action=im_ajax_call&perform=get_results&show_all_filters=true&vin[]='.$vehicle->vin.'">'.$location['name'].'<br/>'.$location['phone'].'</a>';
	}
}
...
function add_popover_to_vdp( $value, $vehicle ){
	...
	{
		return "<div class='popover-container'><a trid='dc0564ed248d4adba55e37' trc role='button' data-toggle='popover' data-trigger='click' data-placement='top' href='#dealer-location-popover' class='btn btn-small btn-default' id='dealer-location-popover-trigger' rel='button'>{$location['name']}</a></div>";
	}
}
		...
		<div class="row location-popover-wrap">
			...
			<?php if ( !empty($address) ): ?>
				<span class="dealer-address">
					<a trid="0b83d3fad31e41deb80bb6" trc href="<?= $google_url ?>" target="_blank" title="View Directions"><?= $address ?></a>
				</span>
			<?php endif; ?>
			</div>
			...
			<?php if ( $location['url'] !== false && !empty($location['url']) ): ?>
				<a trid="411d78d07cbe44c3ac83eb" trc class="btn btn-sm btn-block btn-primary" target="_blank" href="<?= $location['url']; ?>">Visit Site <i class="fa fa-external-link"></i></a>
			<?php endif; ?>
			</div>
		</div>
	</div>
...
function di_get_archives_shortcode ($atts)
{
    ...
    $archive_results_html .= '<li><a trid="91e5e15489124aa192f553" trc href="#">'.$year.'</a>';
    ...
    $archive_results_html.= '<ul class="archive-sub-menu-'.$year.$is_hidden;
        ...
        foreach($months as $month) :
          ...
          $archive_results_html .= '<li><a trid="6210bc7063c84e39aa9b65" trc href="'.$month_link.'"><span>'.$link_date.'</span></a></li>';
...
function fj_show_posts( $atts , $content = null )
{
    ...
    <ul class="posts list-unstyled clearfix <?= ($thumbnail =="true" ? 'withThumbnail ' : '') . ($text != "light" ? $text : "") ?>">
        <?php while ($query->have_posts()) : $query->the_post();
            ...
            <?php }
            if ($thumbnail == 'true') { ?>
                <span class="post-image"><a trid="5ec723e39c86424eaf4512" trc href="<?= get_permalink() ?>"><?= get_the_post_thumbnail( $id, array('auto',50)) ?></a></span>
            <?php } ?>
            <h3><a trid="ca1e3a5cc05a41f488ab89" trc href="<?= get_permalink() ?>"><?= get_the_title() ?></a></h3>
...
<div id="inquire-button">
            <a trid="20fd8a751d8748c59bd2eb" trc href="#inquire-modal" class="inquire button cta-button lightbox fancybox">Inquire</a>
...
function vdp_inquire_cta() {
    ...
    if ($frontend->data['vehicle']['model'] == 'G-Class' || $frontend->data['vehicle']['model'] == 'G-CLASS' || $frontend->data['vehicle']['model'] == 'AMG® GT') {
        ?>
<div id="inquire-button">
            <a trid="4370175dd64242e7a7c239" trc href="#inquire-modal" class="inquire button cta-button lightbox fancybox">Inquire</a>
            <div id="inquire-modal" style="display: none;">
                <?php gravity_form(38, true, false, false, '', false); ?>
            </div>
        </div>
