...
add_action('vehicle_pre_save', function ($vehicle) {
    ...
    if ($vehicle->__image_count < 1 && !empty($placeholderImage)) {
        $vehicle->images = array($placeholderImage);
    }
...
function vrp_options_cta(){
    ...
    if(!isset($vehicle['factory_options']) || empty($vehicle['factory_options']))
        return;
    $html = '<div trid="faecdbf602bb445cb9fb11" trc class="button primary-button block options-button" data-toggle="modal" data-target="#DIModal" data-modal-content="#vrp-vehicle-packages-'.$vehicle['vin'].'" data-modal-title="'.$vehicle['title'].' Options" data-modal-footer="hide">'. do_shortcode('[di_svg name="gear"]') .'View Options</div><div id="vrp-vehicle-packages-'.$vehicle['vin'].'" style="display:none;"><ul>';
    
    foreach($vehicle['factory_options'] as $package){
        if(isset($package['name']) && !empty($package['name']) && !in_array($package['name'],['ADDITIONAL EQUIPMENT','PRIMARY PAINT','SEAT TYPE','SEAT TRIM','WHEELS','SEAT VENTILATION'])){
            $html .= '<li>'.$package['name'].'</li>';
        }else if(isset($package['description']) && !empty($package['description'])){
