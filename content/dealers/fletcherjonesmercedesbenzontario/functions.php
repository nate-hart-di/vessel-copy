...
function dealer_theme_description_space_fix($vehicle)
{
  ...
  if (strpos($vehicle->description, 'P latinum') !== false) {
    $vehicle->description = str_replace(['P latinum', 'E xclusive'], ['Platinum', 'Exclusive'], $vehicle->description);
  }
...
function dealer_theme_used_vrp_cta($cta_buttons)
{
  ...
  if (
    ...
    (!is_numeric($inventoryFrontend->data['vehicle']['our_price']) ||
      (is_numeric($inventoryFrontend->data['vehicle']['our_price']) &&
        $inventoryFrontend->data['vehicle']['our_price'] == 0))
  ) {
            ...
            <?php while ($query->have_posts()):
              ...
              ?>
                ...
                <li class="post">
                    ...
                    <?php }
                    if ($thumbnail == 'true') { ?>
                        <span class="post-image"><a trid="d639fdfb7431439aac42a1" trc href="<?= get_permalink() ?>"><?= get_the_post_thumbnail(
  $id,
  ['auto', 50],
) ?></a></span>
                    <?php }
