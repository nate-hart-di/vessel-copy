...
function add_horsepower_to_inventory($vehicle)
{
  ...
  if(isset($vehicle->tech_options) && !empty($vehicle->tech_options)) {
	...
	if($apiID == 'FJAudiBeverlyHills') {
		$vehicle->VRP_link = '<a trid="80e9338979ba40cfa540bf" trc target="_new" class="vehicle-location-link" href="'. rtrim($location['url'], "/").'/all-inventory/index.htm?search='.$vehicle->vin.'">'.$location['name'].'<br/>'.$location['phone'].'</a>';
	}
	else {
		$vehicle->VRP_link = '<a trid="add4e051770b4104abb0a3" trc target="_new" class="vehicle-location-link" href="'.rtrim($location['url'], '/').'/new-vehicles/#action=im_ajax_call&perform=get_results&show_all_filters=true&vin[]='.$vehicle->vin.'">'.$location['name'].'<br/>'.$location['phone'].'</a>';
	}
}
	...
	<div id="dealer-location-popover" class="hidden">
		<div class="row location-popover-wrap">
			...
			<?php if ( $location['url'] !== false && !empty($location['url']) ): ?>
				<a trid="19a03c4421dc425db151f5" trc class="btn btn-sm btn-block btn-primary" target="_blank" href="<?= $location['url']; ?>">Visit Site <i class="fa fa-external-link"></i></a>
			<?php endif; ?>
			</div>
		</div>
	</div>
...
function di_get_archives_shortcode ($atts)
{
    ...
    $archive_results_html.= '<ul class="archive-sub-menu-'.$year.$is_hidden;
        ...
        foreach($months as $month) :
          ...
          $archive_results_html .= '<li><a trid="3d5a463d38bf45f4900704" trc href="'.$month_link.'"><span>'.$link_date.'</span></a></li>';
...
function fj_show_posts( $atts , $content = null )
{
    ...
    <ul class="posts list-unstyled clearfix <?= ($thumbnail =="true" ? 'withThumbnail ' : '') . ($text != "light" ? $text : "") ?>">
        <?php while ($query->have_posts()) : $query->the_post();
            ...
            <?php }
            if ($thumbnail == 'true') { ?>
                <span class="post-image"><a trid="9e4ae0f26d0c45bd849320" trc href="<?= get_permalink() ?>"><?= get_the_post_thumbnail( $id, array('auto',50)) ?></a></span>
            <?php } ?>
...
add_action('after_fj_header_v2_address',function(){
    ...
    <span class="dealer-locations-phone"><?php echo do_shortcode('[acf field="_dealer_sales_phone" post_id="171"/]'); ?></span>|
    ...
    <span class="dealer-locations-phone"><?php echo do_shortcode('[acf field="_dealer_sales_phone" post_id="169"/]'); ?></span>|
