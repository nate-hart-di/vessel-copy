...
function load_child_scripts()
{
  if (function_exists('load_vehicle_sprites')) {
    load_vehicle_sprites(['honda-menu']);
  }
}
...
function get_site_link_for_dealership($vehicle)
{
  ...
  $urls = [
    'FJHondaHilo' => ['name' => 'Big Island Honda Hilo', 'phone' => get_option('di_phone_sales')],
    'FJHondaKona' => ['name' => 'Big Island Honda Kona', 'phone' => get_option('di_phone_fax')],
  ];
...
add_filter(
  'im_filter_vdp_mainvehicleheaderbox_end',
		...
		<div class="row location-popover-wrap">
			...
			<?php if (!empty($address)): ?>
				<span class="dealer-address">
					<a trid="e29a17c48b7247188f33c6" trc href="<?php echo $google_url; ?>" target="_blank" title="View Directions"><?php echo $address .
  '<br>' .
  $locality; ?></a>
				</span>
			<?php endif; ?>
...
{
  return [['type:New', 'type:Used'], 'make:Honda'];
}
add_action('vrp_listview_pricing_bottom_text', function () {
  global $inventoryFrontend; //End of year disclaimer
  if (
