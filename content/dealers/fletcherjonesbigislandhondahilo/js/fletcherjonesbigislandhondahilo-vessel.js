jQuery(document).ready(function ($) {
  ...
  $('.menu-toggle').click(function () {
    if (typeof 'hj' == 'undefined') hj('trigger', 'menu');
    else console.log('No Hotjar found.');
  ...
  if (isSmallMobile.any() || $(window).width() < 768) {
    ...
    $('a[href^="tel:"], .phone, .sales-phone')
      .not('#mobile-phone-picker a')
      .click(function (e) {
        e.preventDefault();
        $('#mobile-phone-picker').modal();
      });
    $('a[href*="maps.google"], .address')
  ...
  if ($('.inventory-owl #results-page').length > 0) {
    $('body').live('vrp-ready vrp-ajax-complete', function () {
      $('.vehicle').each(function (i, obj) {
        ...
        $(
          "<a trid='4e0cbdaac7924307ac2a82' trc href='/apply-for-financing/' class='button primary-button block'>Get Financing</a>",
        ).appendTo(location);
      });
    });
  }
