jQuery(document).ready(function ($) {
  ...
  $('body').live('vrp-ajax-complete', function () {
    if ($('#menu-overlay').hasClass('open')) closeMenuOverlay();
  });
  ...
  if (isSmallMobile.any() || $(document).width() < 768) {
    ...
    var phone_el = [
      ...
      '.phone-main',
      '.call-us',
      '.mobile-tab-content-inside .phone',
    ];
    $.each(phone_el, function (i, el) {
      if ($(el).length && typeof $(el) != 'undefined') {
        var dealer_phone = $('body')
          .find(el)
          .html()
          .replace(/((\(\d{3}\) ?)|(\d{3}-))?\d{3}-\d{4}/, phone_hypen);
  ...
  if ($('.inventory-owl #results-page').length > 0) {
    $('body').live('vrp-ready vrp-ajax-complete', function () {
      $('.vehicle').each(function (i, obj) {
        ...
        $(
          "<a trid='ef15cb670959458baaa053' trc href='/apply-for-financing/' class='button primary-button block'>Get Financing</a>",
        ).appendTo(location);
      });
    });
  }
