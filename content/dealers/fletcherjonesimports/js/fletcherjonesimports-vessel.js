    ...
    $('body').on('vrp-ready vrp-ajax-complete', function () {
      $('.vehicle').each(function (i, obj) {
        if ($(this).find('.vehicle-price .primary-cta').length == 0) {
          ...
          $(mLocation)
            .append($(finance_button).removeClass('hidden-all'))
            .find('.price-top')
            .prepend(
      ...
      if ($(document).width() < 768 && $('.show-filters-button').length == 0) {
        ...
        $(sort)
          ...
          .append(
            "<a trid='b032cf81ce3f49cd89b60e' trc class='button primary-button show-filters-button align-left' style='margin:0'>Filter Results</a>",
          )
          .find('select')
          .css({ width: '40%' });
