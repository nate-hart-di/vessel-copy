...
function add_mixpanel_events()
{
    if ( DIFunctions::is_inventory_page() === true )
    {
        ...
        $output .= "    jQuery('.vehicle .btn-customize-lease').on('click', function(e) { mixpanel.track('VRP customize lease button'); });\n";
        $output .= "    jQuery('.vehicle .price-leaseandfinance .finance').on('click', function(e) { mixpanel.track('VRP finance amount'); });\n";
        $output .= "    jQuery('.vehicle .price-leaseandfinance .leasepayment').on('click', function(e) { mixpanel.track('VRP lease amount'); });\n";
        $output .= "}\n";
        $output .= "}});\n";
        $output .= "</script>\n";
...
add_filter('im_vrp_filter_array',function($rows,$key){
...
add_filter('algolia_vehicle_types', function($types) {
    return array(array('type:New', 'type:Pre-Owned', 'type:Certified Pre-Owned'));
});
...
add_action('vrp_listview_pricing_bottom_text', function() {
    ...
    <?php elseif (is_in_black_friday_window()
            && DIFunctions::is_inventory_page('new')
            && $inventoryFrontend->vehicle->year == 2018
            && !strpos(strtolower($inventoryFrontend->vehicle->stock), 'cx')) : ?>
        ...
        <?php endif;
    ?><a trid='ed3306b7de24484b83cc87' trc class='button primary-button block trade-link' href='/value-your-trade/'>Value Your Trade</a><?php
        ...
        <ul class="<?= $post_type; ?> posts list-unstyled clearfix <?= ($thumbnail =="true" ? 'withThumbnail ' : '') . ($text != "light" ? $text : "") ?>">
            <?php while ($query->have_posts()) : $query->the_post();
                ...
                <li class="post">
                    ...
                    <?php }
                    if ($thumbnail == 'true') { ?>
                        <span class="post-image"><a trid="c2bf899c51e7464292eabe" trc href="<?= get_permalink() ?>"><?= get_the_post_thumbnail( $id, array('auto',50)) ?></a></span>
                    <?php } ?>
