...
add_action('wp_footer', function() {
    ...
    if( array_key_exists('vehicle', $inventoryFrontend->data) ){
        ...
        $vehicle = $inventoryFrontend->data['vehicle'];
        $form_id = 60;
        echo sprintf('<div id="gravity-form-%d" class="hidden-all metris-form">%s</div>',
            $form_id,
            gravity_form($form_id, 'Lease For Less', apply_filters('show_gravity_form_description_on_vdp', false), false, is_object($vehicle) ? $vehicle['json'] : $vehicle, false, 1000, false)
        );
    }
});
...
add_action('department_hours_closed_text',function($m,$d,$day){
...
add_action('bottom_mobile_content_tab', function() {
    ...
    <div class="carWashHrs">
            <?php echo do_shortcode('[di_display_open_hours departments="Preferred Owners Car Wash" class=dynamic-hours]'); ?>
    </div>
    <?php
});
...
add_filter('algolia_price_field', function($val) { return 'msrp'; });
        ...
        <ul class="<?= $post_type; ?> posts list-unstyled clearfix <?= ($thumbnail =="true" ? 'withThumbnail ' : '') . ($text != "light" ? $text : "") ?>">
            <?php while ($query->have_posts()) : $query->the_post();
                ...
                <li class="post">
                    ...
                    <?php }
                    if ($thumbnail == 'true') { ?>
                        <span class="post-image"><a trid="6078622f4cf24a588785d5" trc href="<?= get_permalink() ?>"><?= get_the_post_thumbnail( $id, array('auto',50)) ?></a></span>
                    <?php } ?>
...
add_action('after_fj_header_v2_address', function(){
    if(get_field('fj_service_street_address', 'option')) {
        $address = '<span>|</span>Service: <a trid="a8ac1d74736245ed915776" trc target="_blank" itemprop="directions" href="%s">%s • %s</a>';
        echo sprintf($address, get_field('fj_service_google_map', 'option'), get_field('fj_service_street_address', 'option'), get_field('fj_service_city_state_zip', 'option'));
    }
});
