...
*/
  ...
  $('body').click(function (e) {
    ...
    if ($(clicked).parents('#menu-overlay').length || $(clicked).parents('.menu-toggle').length) return true;
    else closeMenuOverlay();

    if (
      $(clicked).parents('.overlay-container').length ||
      $(clicked).hasClass('overlay-toggle') ||
      $(clicked).parents('.overlay-toggle').length
    )
      return true;
    else closeToolbarOverlays();
  });
  ...
  $('body').live('vrp-ajax-complete', function () {
    if ($('#menu-overlay').hasClass('open')) $('#menu-close').trigger('click');
  });
...
*/
  ...
  if (isSmallMobile.any()) {
    ...
    $('body').click(function (e) {
      var clicked = e.target;
      if ($(clicked).parents('#sidr').length) return true;
      else $('.close-mobile-wrap').trigger('click');
    });
  }
  ...
  if ($('.inventory-owl #results-page').length > 0) {
    $('body').live('vrp-ready vrp-ajax-complete', function () {
      $('.vehicle').each(function (i, obj) {
        ...
        $(
          "<a trid='cf6494d947694ad19881ce' trc href='/apply-for-financing/' class='button primary-button block'>Get Financing</a>",
        ).appendTo(location);
      });
    });
  }
    ...
    jQuery('body').on('vrp-ready vrp-ajax-complete', function () {
      if ($(document).width() < 768 && $('.show-filters-button').length == 0) {
        ...
        $(sort)
          ...
          .append(
            "<a trid='54fcfac26b22416aa3010b' trc class='button primary-button show-filters-button align-left' style='margin:0'>Filter Results</a>",
          )
          .find('select')
          .css({ width: '40%' });
...
/**
 * SC96962 - The Make was not showing up in the search title on the VRP Page. This function will put in the Make if it
 * is not passed in
 *
 * @param params
 */
