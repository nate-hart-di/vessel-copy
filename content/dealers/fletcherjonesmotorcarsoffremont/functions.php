...
function add_mixpanel_events()
{
    if ( DIFunctions::is_inventory_page() === true )
    {
        $output = '';
        $output .= "<script>\n";
        $output .= "jQuery(document).ready(function($){jQuery(window).load(function () { if(typeof mixpanel != 'undefined'){\n";
...
add_filter('get_vehicle_array', function($vehicle) {
  $executive_arr = array('48491', '48540', '50279');
        ...
        <ul class="<?= $post_type; ?> posts list-unstyled clearfix <?= ($thumbnail =="true" ? 'withThumbnail ' : '') . ($text != "light" ? $text : "") ?>">
            <?php while ($query->have_posts()) : $query->the_post();
                ...
                <li class="post">
                    ...
                    <?php }
                    if ($thumbnail == 'true') { ?>
                        <span class="post-image"><a trid="27daadd37a8b42309570bd" trc href="<?= get_permalink() ?>"><?= get_the_post_thumbnail( $id, array('auto',50)) ?></a></span>
                    <?php } ?>
