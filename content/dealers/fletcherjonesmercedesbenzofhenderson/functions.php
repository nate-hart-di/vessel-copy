    ...
    if(!empty($vehicle->factory_options)) {
        foreach($vehicle->factory_options as $key => $fo) {
            if($fo['option_code'] === 'X30') {
                ...
                $vehicle->factory_options[$key]['header'] = $vehicle->factory_options[$key]['name'] = 'Low & High Range 4x4 Package';
            }
        }
    }
});
        ...
        <ul class="<?= $post_type; ?> posts list-unstyled clearfix <?= ($thumbnail =="true" ? 'withThumbnail ' : '') . ($text != "light" ? $text : "") ?>">
            <?php while ($query->have_posts()) : $query->the_post();
                ...
                <li class="post">
                    ...
                    <?php }
                    if ($thumbnail == 'true') { ?>
                        <span class="post-image"><a trid="f5c243ed1b174bfab0ee25" trc href="<?= get_permalink() ?>"><?= get_the_post_thumbnail( $id, array('auto',50)) ?></a></span>
                    <?php } ?>
