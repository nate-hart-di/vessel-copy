...
$mobile_menu_toggle_attrs =
  isset($mobile_menu_toggle_attrs) && !empty($mobile_menu_toggle_attrs) ? $mobile_menu_toggle_attrs : false;
?>


<div class="menu-top <?php echo $showtitle ? ' with-labels' : ''; ?>">
    <a trid="51e0fc4e2d1746cd8e17b0" trc class="mobile-menu-toggle menu-hamburger" href="#mobile-nav-menu" data-gtm-event="mobileHeaderHamburger" <?php if (
      $mobile_menu_toggle_attrs
    ) {
      echo $mobile_menu_toggle_attrs;
    } ?> >
    ...
    <span class="logo" itemscope itemtype="http://schema.org/Organization">
		<a trid="aa2a4d72bd83497b9e8256" trc itemprop="url" href="<?php echo home_url(); ?>" data-gtm-event="mobileHeaderLogo">
			<img itemprop="logo" src="<?php echo get_stylesheet_directory_uri(); ?>/images/logo-head-mobile.png" alt="<?php echo get_bloginfo(
  'title',
); ?>">
		</a>
    ...
    ?>
            <a trid="32dcceb33be047fc9c369d" trc data-phone="Sales" itemprop="phone" href="tel:<?php echo str_replace(
              [' ', '-', '(', ')', '.'],
              '',
              $phone,
            ); ?>" rel="nofollow" data-gtm-event="mobileHeaderPhone">
		<?php } ?>
                <span class="glyphicon glyphicon-earphone" aria-hidden="true"></span>
		</a>
		<span class="icon-label"><?php echo __(
    apply_filters('header-mobilebasic_call_label', 'CALL US'),
    'commontheme',
  ); ?></span>
	</span>
    ...
    <span class="menu-directions">
		...
		<?php } else { ?>
            <a trid="72f0a6e3b15349daadd93c" trc target="_blank" itemprop="directions" href="<?= function_exists(
              'google_map_mobile_link',
            )
              ? google_map_mobile_link()
              : get_option('di_google_map') ?>" data-gtm-event="mobileHeaderDirections">
		<?php } ?>
                <span class="glyphicon glyphicon-map-marker" aria-hidden="true"></span>
		</a>
		<span class="icon-label"><?php echo __(
    apply_filters('header-mobilebasic_directions_label', 'FIND US'),
    'commontheme',
  ); ?></span>
	</span>
    ...
    <div id="pickNumberMobile">
        ...
        <?php foreach ($phone_numbers as $dept => $phone):
          $number = get_option($phone['option_slug']); ?>
            ...
            <div class="option">
                <a trid="a2b27e90d71649259f396a" trc data-phone="<?= $dept ??
                  'Sales' ?>" itemprop="phone" href="tel:<?= str_replace(
  [' ', '-', '(', ')', '.'],
  '',
  $number,
) ?>" rel="nofollow" data-gtm-event="<?= $phone['gtm_event'] ?>">Call <?= $dept ?? 'Sales' ?> <i class="fa <?= $phone[
   'fa_icon'
 ] ?>"></i></a>
            </div>
