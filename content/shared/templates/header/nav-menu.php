<div id="main-navbar" class="navbar flex">
    ...
    foreach ($items as $i => $menu_item) {
      ...
      ?>
				<li class="menu-item">
                    <a trid="0d3d5af3d8d1481fb671c9" trc <?php if (
                      $class == '' ||
                      in_array('hide-submenu', $menu_item->classes)
                    ): ?>href="<?= $menu_item->url ?>"<?php endif; ?> target="<?php echo $link_target; ?>" ><?= $menu_item->attr_title !=
''
  ? str_replace(' Vehicles', '', $menu_item->attr_title)
  : str_replace(' Vehicles', '', $menu_item->title) ?></a>
