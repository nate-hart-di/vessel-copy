...
{
  return require_once \DealerInspire\Vessel\Vessel::get_plugin_dir_path() .
    'content/shared/templates/homepage/main-ctas.php';
}

function get_imports_homepage_ctas()
{
  return require_once \DealerInspire\Vessel\Vessel::get_plugin_dir_path() .
    'content/shared/templates/homepage/imports-main-ctas.php';
}

function get_homepage_countdown()
{
  return require_once \DealerInspire\Vessel\Vessel::get_plugin_dir_path() .
    'content/shared/templates/homepage/countdown-timer.php';
}

function get_shared_homepage_template($file)
{
  require_once \DealerInspire\Vessel\Vessel::get_plugin_dir_path() .
    'content/shared/templates/homepage/' .
    $file .
    '.php';
}
...
{
  require_once \DealerInspire\Vessel\Vessel::get_plugin_dir_path() .
    'content/shared/templates/mobile/' .
    $file .
    '.php';
}
...
function get_frontend_component($slug, $name = null, $params = [])
{
  if (
    file_exists(\DealerInspire\Vessel\Vessel::get_plugin_dir_path() . 'content/shared/styles/' . $slug . '.min.css')
  ) {
    $styles = file_get_contents(
      \DealerInspire\Vessel\Vessel::get_plugin_dir_path() . 'content/shared/styles/' . $slug . '.min.css',
    );
...
{
  $should_show_on = ['cla', 'c-class', 'slc', 'gla', 'glc', 'e-class', 'gls', 'cls', 's-class', 'amg® gt', 'g-class'];
...
function show_social_icons(array $social_networks)
{
  foreach ($social_networks as $network => $url) {
    if (!empty(get_option('di_' . $network . '_url')) && $network !== 'twitter') {
      ...
      continue;
    } elseif (!empty(get_option('di_twitter_id')) && $network === 'twitter') {
...
if (!function_exists('get_scoped_shared_template')) {
   ...
   */
  function get_scoped_shared_template($slug, $name = null, $params = [], $_return = false)
  {
