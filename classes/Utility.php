...
class Utility
{
    public static $token = 'xoxp-3964641230-75341862818-298670746017-6420fa6e98a91ea0824537b7372bc640';
    ...
    public static function set_vessel_version() {
        ...
        $version = get_file_data($plugin, array('Version'), 'plugin');
    ...
    public static function check_vessel_version() {
            ...
            $vessel_version = get_file_data($vessel_plugin, array('Version'), 'plugin');
        ...
        if (version_compare($old_version, $current_version, '<') && self::$vessel->is_prod()) {
                $message = '<!here> | Vessel updated on *' . get_option('di_description') . '* from `' . $old_version . '` to `' . $current_version . '` on ' . date('m/d/y') . ' at ' . date('H:i:s');
