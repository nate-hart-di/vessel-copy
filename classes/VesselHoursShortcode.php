...
class VesselHoursShortcode
{
        ...
        public function init(){
            ...
            add_shortcode( 'di_display_open_hours', array($this,'do_my_shortcode') );
        }
		...
		public function do_my_shortcode($atts, $content = null) {
			...
			foreach($hours as $h){
				...
				if(in_array($h['name'], $departments))
					$return_string .= '<span class="department">'._x( $h['name'], 'Dynamic Hours', 'dealerinspire' ).':&nbsp;</span><span class="hours">';
				else
					continue;
				...
				if($todays_hours != 'Closed') {
					$is_open = true;
					$open = strtolower($todays_hours['open_hour'].($todays_hours['open_minute'] > 0 ? ':'.$todays_hours['open_minute'] : '').$todays_hours['open_meridian']);
					$close = strtolower($todays_hours['close_hour'].($todays_hours['close_minute'] > 0 ? ':'.$todays_hours['close_minute'] : '').$todays_hours['close_meridian']);
                    ...
                    $return_string .= '<span class="doy">'._x( substr($current_day,0,3), 'Dynamic Hours', 'dealerinspire' ).'&nbsp;</span>';
					$return_string .= $open.$time_separator.$close.((!empty($disclaimer) && $show_disclaimer == true) ? ' <span class="disclaimer">('.$disclaimer.')</span>' : '');

				} else {
					// Allow filters to customize the 'Closed' text based on the Department and Day of the week.
					$closed_text = apply_filters('department_hours_closed_text', _x('Closed', 'DI Hours', 'dealerinspire'), $h['name'], $current_day);
			...
			$open_today = $is_open ? '<span class="open">' . apply_filters('open_today_text', __('Open Today!', 'dealerinspire')) . '</span> ' : '';
