...
class Enqueue {
    ...
    public function vessel_actions_filters() {
        ...
        add_action('init', array($this, 'register_shared_shortcodes'));
        add_action('wp_print_scripts', array($this,'override_js'));

        //Switch to the homepage, styles and functions file inside Vessel

        if ( Vessel::is_default_frontpage() ){
            add_filter( 'template_include', array($this,'use_vessel_homepage'), 99);
        }
        add_filter( 'template_include', array($this,'use_new_single_template'), 99);
        add_filter( 'template_include', array($this,'use_new_blog_template'), 99);
        add_action('wp_enqueue_scripts', array($this, 'enqueue_shared_vessel_stylesheet'), 21);
        if(!$this->vessel->ShouldUseDealerThemeFiles( $this->_fj->get_di_id() ) ){
            add_action('wp_enqueue_scripts', array($this, 'enqueue_vessel_styles'), 20);
            add_action('wp_enqueue_scripts', array($this, 'enqueue_vessel_scripts'));
            add_action('wp_after_body', array($this, 'return_new_header_template'), 1);
            add_action('wp_footer', array($this, 'return_new_footer_template'), 9999999999999999999);
        }
        add_filter('di_stacks_top', array($this, 'addTopStacks'));


        add_action('wp_head', array($this, 'enqueue_shared_js'));
        add_action('init', array($this, 'enqueue_vessel_functions_file'), 1);

        //Dequeue default CSS stylesheet from DealerTheme
        add_action('wp_enqueue_scripts', array($this, 'dequeue_default_css'), 99);
    ...
    public function register_shared_shortcodes() {
        add_shortcode('fj_holiday_hours', array($this, 'holiday_hours_shortcode'));
    }
    ...
    public function holiday_hours_shortcode(){
        ...
        <?php
        foreach($hol_sales_hours as $day => $time) {
            ?>
        ...
        <?php
        foreach($hol_service_hours as $day => $time) {
            ?>
            <div class="department-hours">
                <span><?= ucwords($day) ?>: </span><span><?= $time ?></span>
            </div>
            ...
            acf_add_local_field_group(array (
                ...
                'fields' => array (
                    ...
                    array (
                        ...
                        'type' => 'image',
                        'instructions' => 'Header image (example: breast cancer awareness month ribbon). NOTE: Image height is limited with CSS to 55px maximum, but will auto scale its width so it scales.',
                        'required' => 0,
                        'conditional_logic' => 0,
                        'wrapper' => array (
                            'width' => '',
                    ...
                    array (
                        ...
                        'type' => 'image',
                        'instructions' => 'Footer image (example: breast cancer awareness month ribbon). NOTE: Image height is limited with CSS to 55px maximum, but will auto scale its width so it scales.',
                        'required' => 0,
                        'conditional_logic' => 0,
                        'wrapper' => array (
                            'width' => '',
...
You can choose a hex value or color. Ex: #FFFFFF or red',
            ...
            acf_add_local_field_group(array (
                ...
                'fields' => array (
                    array (
                        ...
                        'name' => 'holiday_hours',
                        'type' => 'repeater',
                        'instructions' => 'Generates holiday hours text to be used with the [fj_holiday_hours /] shortcode. <b>First row is treated as Sales, second row is treated as Service.</b> Produces format of Day: 9:00 AM - 10 PM',
                        'required' => 0,
                        'conditional_logic' => 0,
                        'wrapper' => array (
                            'width' => '',
    ...
    public function dequeue_default_css() {
        ...
        $enqueued_dealertheme_styles = array(
    ...
    public function flipclock_file_hooks() {
        ...
        wp_enqueue_script('flipclockjs', null, array('jquery'), null, true);
    }

    public function flipclock_hooks() {
            add_action('wp_enqueue_scripts', array($this, 'enqueue_flipclock_assets'));
            add_action('wp_enqueue_scripts', array($this, 'flipclock_file_hooks'));
    ...
    public function enqueue_flipclock_assets() {
        ...
        $countdown_timer_assets= array(
            'vesselEndDate' => $end_date
        );

        wp_register_script('vessel-flipclock', plugins_url() . '/vessel/content/shared/flipclock/homepage-timer.js');
        wp_enqueue_script('vessel-flipclock', plugins_url() . '/vessel/content/shared/flipclock/homepage-timer.js', array('jquery'), null, true);
    ...
    public function enqueue_vessel_scripts() {
        ...
        wp_enqueue_script('jquery-ui-accordion', false, array(
            'jquery',
            'jquery-ui-core'
        ));
        ...
        if (!$this->vessel->is_prod()) {
            wp_register_script(
                ...
                plugins_url() . '/vessel/content/dealers/'.$this->_fj->get_di_slug() . '/js/' . $this->_fj->get_di_slug() . '-vessel.js',
                array('jquery'),
                filemtime($this->vessel->get_plugin_path() . 'content/dealers/' . $this->_fj->get_di_slug() . "/js/" . $this->_fj->get_di_slug() . '-vessel.js'),
                true
            );
        ...
        if (file_exists($this->vessel->get_plugin_path() . 'content/dealers/' . $this->_fj->get_di_slug() . "/js/min/" . $this->_fj->get_di_slug() . '-vessel.min.js')) {
            wp_register_script(
                $this->_fj->get_di_slug(). '-script',
                plugins_url() . '/vessel/content/dealers/'.$this->_fj->get_di_slug() . '/js/min/' . $this->_fj->get_di_slug() . '-vessel.min.js',
                array('jquery'),
                filemtime($this->vessel->get_plugin_path() . 'content/dealers/' . $this->_fj->get_di_slug() . "/js/min/" . $this->_fj->get_di_slug() . '-vessel.min.js'),
                true
            );
            wp_enqueue_script($this->_fj->get_di_slug() . '-script');
        }
    ...
    public function enqueue_shared_js(){
        if (file_exists($this->vessel->get_plugin_path() . 'content/shared/min/shared-scripts.min.js')) {
            ...
            wp_localize_script($this->_fj->get_di_slug(). '-shared-scripts','variables', [ 'ajaxurl' => admin_url('admin-ajax.php'), 'shuttle_form'=>$shuttle_form ,'is_MB' =>$is_MB_site]);
    ...
    public function enqueue_vessel_styles() {
        ...
        if (file_exists($dealerRootCSS . $this->_fj->get_di_slug() . ".min.css")) {
            wp_enqueue_style('vessel-style-css-home', plugins_url() . '/vessel/content/dealers/' . $this->_fj->get_di_slug() .'/css/' . $this->_fj->get_di_slug() . '.min.css', array(), filemtime($this->vessel->get_plugin_path() . 'content/dealers/' . $this->_fj->get_di_slug() . "/css/" . $this->_fj->get_di_slug() . ".min.css"));
        }

        if ( file_exists($dealerRootCSS . "home.min.css") && is_front_page() ) {
            wp_enqueue_style('vessel-base-child-style-home', plugins_url() . '/vessel/content/dealers/' . $this->_fj->get_di_slug() .'/css/home.min.css', array(), filemtime($this->vessel->get_plugin_path() . 'content/dealers/' . $this->_fj->get_di_slug() . "/css/home.min.css"));
        }

        if ( (file_exists($dealerRootCSS  . "vrp.css") || file_exists($dealerRootCSS  . "vrp.min.css") ) && \DIFunctions::is_inventory_page()) {
            wp_enqueue_style('vessel-base-child-style-vrp', plugins_url() . '/vessel/content/dealers/' . $this->_fj->get_di_slug() .'/css/vrp.min.css', array(), filemtime($this->vessel->get_plugin_path() . 'content/dealers/' . $this->_fj->get_di_slug()  . "/css/vrp.min.css"));
        }

        if ( (file_exists($dealerRootCSS . "vdp.css") || file_exists($dealerRootCSS . "vdp.min.css") )  && \DIFunctions::is_vehicle_page() ) {
            wp_enqueue_style('vessel-base-child-style-vdp', plugins_url() . '/vessel/content/dealers/' . $this->_fj->get_di_slug() .'/css/vdp.min.css', array(), filemtime($this->vessel->get_plugin_path() . '/content/dealers/' . $this->_fj->get_di_slug() . "/css/vdp.min.css"));
        }

        if ( (file_exists($dealerRootCSS . "inside.css") || file_exists($dealerRootCSS . "inside.min.css") )  && \DIFunctions::is_vehicle_page() ) {
            wp_enqueue_style('vessel-base-child-style-vdp', plugins_url() . '/vessel/content/dealers/' . $this->_fj->get_di_slug() .'/css/inside.min.css', array(), filemtime($this->vessel->get_plugin_path() . '/content/dealers/' . $this->_fj->get_di_slug() . "/css/inside.min.css"));
        }

        if (
        file_exists($this->vessel->get_plugin_path() . 'content/dealers/' . $this->_fj->get_di_slug() . "/css/inside.min.css")
        ) {
            if ((
                    (! is_front_page())
                ...
                wp_enqueue_style('vessel-base-child-style-inside', plugins_url() . '/vessel/content/dealers/' . $this->_fj->get_di_slug() .'/css/inside.min.css', array(), filemtime($this->vessel->get_plugin_path() . 'content/dealers/' . $this->_fj->get_di_slug() . "/css/inside.min.css"));
            }
        }
    }
    ...
    function addTopStacks($stacks){
        ...
        if (
            !isUsingSiteBuilderHeader()
            && \get_option('dealerinspire-assets_di_stacks_convert_header', false) )
        {
            $stacks[] = [
                'id' => 'header-content',
    ...
    public function enqueue_vessel_functions_file() {
        ...
        if (file_exists($this->vessel->get_plugin_path() . 'content/dealers/' . $this->_fj->get_di_slug() . "/functions.php")) {
            include_once $this->vessel->get_plugin_path() . '/content/dealers/' . $this->_fj->get_di_slug() . '/functions.php';
        }
    }
    ...
    public function add_mb_styles() {
        ...
        wp_register_style('mercedesfont-ca', plugins_url() . '/vessel/content/shared/styles/mercedes-benz-fonts-canada.css');
        wp_enqueue_style('mercedesfont-ca');
    }

    public function mb_actions_filters() {
        add_action('wp_enqueue_scripts', array($this, 'add_mb_styles'), 999999999999999);
    }
