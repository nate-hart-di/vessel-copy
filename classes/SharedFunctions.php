...
class SharedFunctions
{
  ...
  public function add_header_hooks()
  {
    ...
    add_action('init', [$this, 'new_thumbnail_img'], 15);
    add_filter('enable_di_back_button', [$this, 'back_button'], 30, 1);
    add_action('wp_enqueue_scripts', [$this, 'load_shared_scripts']);
    add_action('wp_enqueue_scripts', [$this, 'add_custom_conversations_styles']);
    add_action('wp_enqueue_scripts', [$this, 'script_dequeues'], PHP_INT_MAX);
    add_filter('algolia_inventory_synonyms', [$this, 'vessel_inventory_synonyms'], 1, 1);
    add_action('porsche_top_item', [$this, 'google_translate'], 1, 1);
  }
  ...
  public function add_body_hooks()
  {
    ...
    add_filter('body_class', [$this, 'add_class_to_specials_page'], 10, 1);
    add_filter('body_class', [$this, 'add_3rd_party_class'], 10, 1);
    add_filter('body_class', [$this, 'add_vrp_class'], 10, 1);
    add_filter('body_class', [$this, 'add_service_class'], 10, 1);
    add_action('wp_footer', [$this, 'di_archives_shortcode_js'], 10);
    add_filter('im_vdp_hotwheels_ctabox_bottom_offset', [$this, 'ctabox_footer_offset']);

    add_action('wp_footer', [$this, 'dipc_lightning_mobile_filter']);
    add_action('mobile_tabs_contact_add_phone_numbers', [$this, 'mobile_phone_numbers']);
    add_action('bottom_mobile_content_tab', [$this, 'mobile_content_tab_bottom_content']);

    add_action('blog_archive_before_main_loop', [$this, 'blog_archive_before_main_loop']);
  }
  ...
  {
    // Inventory Hooks
    add_action('im_homepage_search_filter_vehicle_attributes', [$this, 'use_chrome_trim'], 10, 1);
    add_action('vehicle_pre_save', [$this, 'inventory'], 11, 1);
    if (!in_array($this->_fj->get_di_id(), [571, 535])) {
      // Exclude MB Ontario & MB Chicago, all VRP CTAs in Action Links
      add_action('post_button_bar_buttons', [$this, 'post_button_bar_buttons'], 10, 1);
    }
    add_filter('vdp_link_url_variables', [$this, 'vdp_link_url_variables'], 10, 1);
    add_filter('im_premium_options_keys', [$this, 'expand_premium_features_search_values']);
    add_filter('im_premium_options_list', [$this, 'add_premium_options'], 10, 1);
    add_filter('get_vehicle_array', [$this, 'amg_vehicle_titles'], 12);
    add_filter('algolia_vehicle_types', [$this, 'set_algolia_vehicle_types']);
    add_filter('im_sold_vehicle_redirect_url', [$this, 'set_sold_vehicle_redirect_url']);
    add_filter('algolia_inventory_synonyms', [$this, 'algolia_synonyms'], 99, 1);
    add_filter('vehicle_pre_save', [$this, 'add_amg_line_package_to_vrp_feature_filter']);
    add_filter('di_google_events_form_type_mapping', [$this, 'change_form_tracking_types']);

    // VDP Hooks
    if (is_prod() && $this->_fj->get_di_id() === 435) {
      add_filter('di_hotwheels_gallery_path', [$this, 'hotwheels_gallery_path']);
    }

    add_filter('di_hotwheels_show_lightbox_gallery', [$this, 'use_vdp_gallery_lightbox']);

    add_filter('hotwheels_below_cta_box', [$this, 'hotwheels_below_cta_box'], 10, 1);
    add_filter('di_common_svg_dirs', [$this, 'di_common_svg_dirs'], 10, 1);

    // Misc Hooks
    add_filter('special_service_layout', [$this, 'coupon_service_template'], 999, 3);
    //add_filter('user_can_richedit', array($this, 'remove_rich_edit_access'));
    add_filter('text_specials_print_button', [$this, 'change_print_button_text'], 10, 3);
  ...
  public function register_sidebars()
  {
    ...
    if ($this->_fj->get_di_id() != 797) {
      // Exclude Big Island Honda because we need 2 different hours blocks; one for each store
      register_sidebar([
        'name' => 'Holiday Hours',
        'id' => 'holiday-hours',
      ...
      // These are the Hoilday Hours blocks for Big Island Honda
      register_sidebar([
  ...
  public function register_shared_acf_groups()
  {
    ...
    if (
      $id == $this->_fj->dealerIdList['fjmercedes'] ||
    ...
    if ($id == $this->_fj->dealerIdList['fjmaui'] || $id == $this->_fj->dealerIdList['fjhonolulu']) {
  ...
  public function add_premium_options($list)
  {
    $features = [
      [
        ...
        'keywords' => ['amg line', 'amg line package'],
      ],
    ];
  ...
  {
    add_shortcode('show-posts', [$this, 'show_posts']);
    add_shortcode('show-video-gallery', [$this, 'video_gallery']);
    add_shortcode('di_get_archives', [$this, 'di_get_archives_shortcode']);
    add_shortcode('reviews-row', [$this, 'homepage_reviews']);
    add_shortcode('homepage-search-widget', [$this, 'homepage_search_widget']);
  }
  ...
  public function show_posts($atts, $content = null)
  {
    ...
    if ($query->have_posts()) {
      ob_start(); ?>
            ...
            <ul class="posts list-unstyled clearfix <?php echo ($thumbnail == 'true' ? 'withThumbnail ' : '') .
              ($text != 'light' ? $text : ''); ?>">
                <?php while ($query->have_posts()):
                  ...
                  ?>
                    ...
                    <li class="post">
                        ...
                        <?php }
                        if ($thumbnail == 'true') { ?>
                            <span class="post-image"><a trid="e546e1679bd1464c95bc37" trc href="<?php echo get_permalink(); ?>"><?php echo get_the_post_thumbnail(
  $id,
  ['auto', 50],
); ?></a></span>
                        <?php }
   ...
   */
  public function change_print_button_text($print_button_text, $special_id, $ctatext1)
  {
  ...
  public function add_class_to_specials_page($classes)
  {
    ...
    if (\DIFunctions::is_vehicle_page() === true) {
      if (
        isset($inventoryFrontend->data['vehicle']['special_field_1']) &&
        $inventoryFrontend->data['vehicle']['special_field_1'] === 'S'
      ) {
  ...
  public function post_button_bar_buttons($vehicle)
  {
    ...
    $use_ag_data_or_href =
      class_exists('FJAutogravity') &&
      isset($vehicle['autogravity_json']) &&
      !empty($vehicle['autogravity_json']) &&
      strtolower($vehicle['type']) == 'new';
    ...
    echo "<a trid='c891e83c2c4a42deaff21c' trc class='button primary-button block finance-link hidden-all below-price-cta'" .
      $attr .
      '>Get Pre-Approved</a>';
  }
  ...
  public function coupon_service_template($post_content, $coupon_id, $coupon_data)
  {
    $post_content = str_replace(
      ...
      '<a class="view-disclaimer-link">View Disclaimer</a><div class="disclaimer"><i class="fa fa-close"></i>',
      $post_content,
    );
    return $post_content;
  }
  ...
  public function hotwheels_below_cta_box($vehicle)
  {
    ...
    ?>
        ...
        if (
          !empty($premium_options) &&
          is_array($premium_options) &&
          (get_option('di_id') != 654 && get_option('di_id') != 421 && get_option('di_id') != 796)
        ) { ?>
            <div id="ctabox-premium-features" class="ctabox-row open">
                ...
                <div class="premium-options-list">
                    <?php foreach ($premium_options as $premium_option) {
                      echo sprintf(
                        '<li class="list-group-item"><h3>%s%s</h3><p>%s</p></li>',
                        is_array($premium_option)
                          ? (isset($premium_option['name'])
                            ? $premium_option['name']
                            : $premium_option['header'])
                          : $premium_option,
                        is_array($premium_option) && !empty($premium_option['price'])
                          ? ' <span class="vehicle-package-price "> $' .
                            number_format($premium_option['price'], 0) .
                            '</span>'
                          : '',
  ...
  {
    $premium_options = [];
    foreach (['factory_options', 'premium_options'] as $key) {
      if (!empty($vehicle[$key]) && is_array($vehicle[$key])) {
        foreach ($vehicle[$key] as $option) {
          if (
            is_array($option) &&
            isset($option['name']) &&
            in_array(strtoupper($option['name']), ['TRANSMISSION', 'PRIMARY PAINT', 'ENGINE', 'WHEELS']) !== false &&
            empty($option['price'])
          ) {
            continue;
          }
          ...
          foreach ($premium_options as $premium_option) {
            if (
              isset($option['option_code']) &&
              isset($premium_option['option_code']) &&
              $option['option_code'] == $premium_option['option_code']
            ) {
              $duplicate = true;
              break;
            }
  ...
  {
    return array_merge($dirs, [WP_CONTENT_DIR . '/themes/DealerInspireCommonTheme/includes/svg/sharp_and_thin']);
  ...
  private $_carousel = [
    ...
    'custom' => [
      // ACF stock repeater (client-chosen vehicles)
      777, // MB Honolulu
    ],
  ];
  ...
  public function below_homepage_video()
  {
    ...
    if (in_array($this->_fj->get_di_id(), $this->_carousel['used-cpo'])) {
      ...
      $s->type = ['Used', 'Certified Used'];

      if ($this->_fj->get_di_id() == 797) {
        // Compliance fix for Honda
        $s->make = ['Honda'];
      }
    } elseif (in_array($this->_fj->get_di_id(), $this->_carousel['custom'])) {
      // 777 is MB Honolulu
    ...
    } elseif (in_array($this->_fj->get_di_id(), $this->_carousel['cpo'])) {
      // cpo-only
      $s = new InventorySearch();
      $s->type = ['Certified Used'];
      $s->special_field_1 = 'S';
    }
                                ...
                                foreach ($vehicles as $v): ?>
                                    <li class="special-offer card hover-effect per-row-3">
                                        <a trid="2bd6c63afc9947ee916279" trc href="<?php echo $v['link']; ?>" >
                                            <img src='<?php echo $v['thumbnail']; ?>' alt="<?php echo esc_attr(
  $v['title'],
); ?>" />
                                        </a>
                                            ...
                                            <a trid="72b1f964b75b4109b3a0d0" trc href="<?php echo $v['link']; ?>">
                                                ...
                                                <div class="rates">
                                                    <div class="offeritem">
                                                        ...
                                                        <span class="offerlabel"><?php echo is_numeric($v['our_price'])
                                                          ? '$' . number_format($v['our_price'], 0)
                                                          : $v['our_price']; ?></span>
                                                    </div>
                                                </div>
                                        ...
                                        <div class="offerbuttonbar card-action">
                                            <a trid="0671eb76b8f8485a98dd80" trc class="button primary-button block" href="<?php echo $v[
                                              'link'
                                            ]; ?>">View Vehicle</a>
                                        </div>
                                    </li>
    ...
    if (isset($years)) {
      ...
      foreach ($years as $year):
        ...
        foreach ($months as $month):
          ...
          $archive_results_html .=
            '<li><a trid="ae47af2bce0e4c92a1a315" trc href="' .
            $month_link .
            '"><span>' .
            $link_date .
            '</span></a></li>';
  ...
  {
    // SC 00109547
    return [['type:New', 'type:Used', 'type:Certified Used', 'type:Certified Pre-Owned', 'type:Pre-Owned']];
  }
  ...
  {
    $syns[] = ['a class', 'a-class'];
    $syns[] = ['c class', 'c-class'];
    $syns[] = ['e class', 'e-class'];
    $syns[] = ['g class', 'g-class'];
    $syns[] = ['gl class', 'gl-class'];
    $syns[] = ['m class', 'm-class'];
    $syns[] = ['s class', 's-class'];
    $syns[] = ['sl class', 'sl-class'];
    return $syns;
  }
  ...
  public function fixedops_button_classes($classes)
  {
    ...
    foreach ($classes as $class) {
      ...
      } elseif (strpos($class, 'parts') !== false) {
  ...
  {
    $id = $this->_fj->get_di_id();
    $dealerList = ['fjfremont', 'fjhenderson', 'fjimports', 'fjhonolulu', 'fjmaui'];
    foreach ($dealerList as $dealer) {
      if ($id == $this->_fj->dealerIdList[$dealer]) {
        return [
          [
            'label' => 'Sales',
            'number' => get_option('di_phone_mobile_sales')
              ? get_option('di_phone_mobile_sales')
              : get_option('di_phone_sales'),
          ],
          [
            'label' => 'Service',
            'number' => get_option('di_phone_mobile_service')
              ? get_option('di_phone_mobile_service')
              : get_option('di_phone_service'),
          ],
        ];
      }
    }
  ...
  public function mobile_content_tab_bottom_content()
  {
    ...
    foreach ($dealerList as $dealer):
      if ($id == $this->_fj->dealerIdList[$dealer]): ?>
                <div class="ourhours">
                    <p class="service"><?php echo do_shortcode(
                      '[di_display_open_hours departments="Service & Parts" class=dynamic-hours]',
                    ); ?></p>
                </div>
            <?php endif;
    endforeach;
  ...
  {
    $active = str_replace(['/', 'category'], '', $_SERVER['REQUEST_URI']);
    if ($slug == $active) {
      return 'active';
    }
    return '';
  ...
  function blog_archive_before_main_loop()
  {
    if (get_field('use_custom_blog_template', 'option')) {
      ...
      echo '<a trid="8ef698f6d629413fb86b7d" trc class="featuredCategory ' .
        $this->is_active('blog') .
      ...
      foreach ($posts as $post) {
        ...
        $image_tag =
          '<div class="item featuredImage" style="background-image:url(' .
          $image['url'] .
          ');" > ' .
          $content .
          '</div>';
  ...
  public function add_custom_conversations_styles()
  {
    if (get_field('disable_proactive_message', 'option')) {
      wp_register_style(
        ...
        plugins_url() . '/vessel/content/shared/styles/vdp/custom-conversations.css',
      );
    ...
    try {
      ...
      if (isset($file_url)) {
        ...
        foreach ($processed as &$item) {
          ...
          $synonymArray = [$item, $nospacesItem];
          ...
          foreach ($synonymArray as &$entry) {
            ...
            if (strpos($entry, '-') !== false) {
              $synonymArray[] = str_replace('-', '', $entry);
            }
          }
  ...
  public function vessel_inventory_synonyms($syns)
  {
    ...
    if (class_exists('\\di_transient') && method_exists('\\di_transient', 'remember')) {
      $synonymList = \di_transient::get_instance()->remember(
        'synonym-data',
        function () {
