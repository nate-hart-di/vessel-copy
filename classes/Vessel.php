...
class Vessel
{
    ...
    foreach ($this->_settings as $tab_key => $sections) {
      foreach ($sections as $section_key => $section) {
        foreach ($section['settings'] as $key => $setting) {
          register_setting(
            $this->_option_group_prefix . $tab_key,
            $this->_option_prefix . $key,
            array_key_exists('sanitize_function', $setting) ? [$this, $setting['sanitize_function']] : null,
          );
        }
      }
    }
  ...
  {
    return in_array(get_option('page_on_front'), [3, 4, 5]) ? true : false;
  }
  ...
  public function field_callback($args)
  {
    ...
    switch ($args['setting']['type']) {
      case 'textarea':
        echo $this->scoped_read($this->get_plugin_path() . 'views/fields/textarea.php', $params);
        break;
      case 'checkbox':
        echo $this->scoped_read($this->get_plugin_path() . 'views/fields/checkbox.php', $params);
        break;
      case 'hours':
        echo $this->scoped_read($this->get_plugin_path() . 'views/fields/hours.php', $params);
        break;
      case 'select_livechat_tool':
      case 'select_livechat_override':
        echo $this->scoped_read(
          $this->get_plugin_path() . 'views/fields/' . $args['setting']['type'] . '.php',
          $params,
        );
        break;
      default:
        echo $this->scoped_read($this->get_plugin_path() . 'views/fields/text.php', $params);
        break;
    }
  }
   ...
   * @return void
   */
  public function scoped_read($file, $params = [])
  {
    if (file_exists($file)) {
      extract($params);
      ob_start();
  ...
  {
    $settings_link =
      '<a trid="d17303fb950b4916ac1bfc" trc href="admin.php?page=' . $this->_settings_page_slug . '">Settings</a>';
